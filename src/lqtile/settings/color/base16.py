...
from ..default import DEFAULT_BASE16_PALETTE


def load_base16_color_scheme(
    scheme_file: str,
    scheme_folder: str | None = None,
) -> dict | None:
...
def theme_base16_color_palette(color_data: dict) -> dict:
    if color_data["color"].get("base16", None) is None:
        return DEFAULT_BASE16_PALETTE  # type: ignore

    base16_palette = None
    if color_data["color"]["base16"].get("palette", None) is None:
        scheme_dir = color_data["color"]["base16"].get("scheme_dir", None)
        scheme_name = color_data["color"]["base16"].get("scheme_name", None)
        if scheme_name is not None and scheme_dir is not None:
            base16_palette = load_base16_color_scheme(scheme_name, scheme_dir)
