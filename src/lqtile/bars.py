...
from .settings.model import Theme
from .settings.theme.color.iter import widget_color_iter


def build_top_bar(settings: Settings, theme: Theme) -> QBar | None:
    named_colors = theme.color.named
    _color_iter = widget_color_iter(named_colors)
    ...
    start.append(UserMenu(user_menu_context))

    group_box_context = ModuleContext(
        bar_context,
        props={
            "background": named_colors.panel_bg,
        },
    )
    start.append(GroupBox(group_box_context))

    current_layout_context = ModuleContext(
        bar_context,
        props={
            "background": named_colors.panel_bg,
        },
    )
    start.append(CurrentLayout(current_layout_context))
...
def build_bottom_bar(settings: Settings, theme: Theme) -> QBar | None:
    ...
    start.append(Bluetooth(bluetooth_context))
